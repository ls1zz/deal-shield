...
function SignInPageContent() {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    // If there's a session_id, link the subscription
  ...
  const handleGoogleSignIn = async () => {
    ...
    // If there's a session_id, pass it through OAuth flow
      ...
      <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
          ...
          {sessionId && (
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                ✓ Payment confirmed! Sign in to activate your subscription
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600">
            ...
            <Link
              href={sessionId ? `/auth/success?session_id=${sessionId}` : "/sign-up"}
              className="text-blue-600 hover:text-blue-700 font-semibold"
            >
              {sessionId ? "Create Account" : "Sign Up"}
            </Link>
          </p>
        </div>
