...
interface NDAUploadFormProps {
  userId: string
}

export default function NDAUploadForm({ userId }: NDAUploadFormProps) {
  ...
  const processDocument = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/verify/nda', {
        method: 'POST',
        body: formData
      })
      ...
      setTimeout(() => {
        router.push(`/verify/nda/${results.verificationId}`)
      }, 1000)
      ...
      {analysisResults && (
        <div className="p-8 rounded-2xl bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-50 border-2 border-emerald-300 text-center">
          ...
          <p className="text-[16px] text-emerald-700 mb-4">
            Your NDA has been verified. Redirecting to results...
          </p>
